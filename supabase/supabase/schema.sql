...
create policy "Public profile categories are readable"
on public.profile_categories
for select
to authenticated
using (is_public = true or (select auth.uid()) = user_id);

drop policy if exists "Users can create own profile categories" on public.profile_categories;
create policy "Users can create own profile categories"
on public.profile_categories
for insert
to authenticated
with check ((select auth.uid()) = user_id);

drop policy if exists "Users can update own profile categories" on public.profile_categories;
create policy "Users can update own profile categories"
on public.profile_categories
for update
to authenticated
using ((select auth.uid()) = user_id)
...
drop policy if exists "Users can delete own profile categories" on public.profile_categories;
create policy "Users can delete own profile categories"
on public.profile_categories
for delete
to authenticated
using ((select auth.uid()) = user_id);
...
drop policy if exists "Users can read own ad campaigns" on public.ad_campaigns;
create policy "Users can read own ad campaigns"
on public.ad_campaigns
for select
to authenticated
using ((select auth.uid()) = user_id);

drop policy if exists "Users can create own ad campaigns" on public.ad_campaigns;
...
using (
  exists (
    ...
    and (select auth.uid()) = any(chats.participant_ids)
  )
);
...
drop policy if exists "Users can read reactions" on public.reactions;
...
drop policy if exists "Users can read comment reactions" on public.comment_reactions;
...
drop policy if exists "Users can read related follows" on public.follows;
create policy "Users can read related follows"
on public.follows
for select
to authenticated
...
drop policy if exists "Users can create own follow requests" on public.follow_requests;
...
drop policy if exists "Requesters can delete own follow requests" on public.follow_requests;
create policy "Requesters can delete own follow requests"
on public.follow_requests
for delete
to authenticated
...
drop policy if exists "Users can create own legal consents" on public.legal_consents;
...
for select
to authenticated
using (status = 'active' or (select auth.uid()) = broadcaster_id);
...
for insert
to authenticated
with check ((select auth.uid()) = actor_id or (select auth.uid()) = user_id);
...
drop policy if exists "Active preset avatars are readable" on public.preset_avatars;
create policy "Active preset avatars are readable"
on public.preset_avatars
for select
to authenticated
...
using (bucket_id = 'spicey-media');

drop policy if exists "Users can upload own media folder" on storage.objects;
create policy "Users can upload own media folder"
on storage.objects
for insert
to authenticated
with check (
  bucket_id = 'spicey-media'
